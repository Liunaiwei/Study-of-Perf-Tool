...
//	void *data_mmap = (void *)((size_t)our_mmap);
	...
	while(offset<size) {
		...
		/* Sample */
 			 case PERF_RECORD_SAMPLE: {
                        ...
                        if(!quiet) printf("\tPERF_SAMPLE_IDENTIFIER: %llx\n", abi);
                        offset+=8;
...
void fizz_handler(int signum, siginfo_t* info, void* p) 
{
 ...
 // fprintf(stderr, "Hihi, i is %d. P %p si_fd %d\n", i, p, info->si_fd-3);
...
{  
	...
	our_mmap[k]=mmap(NULL, 528384, PROT_READ|PROT_WRITE, MAP_SHARED, perf_fd[k], 0);
...
// Set a signal handler for SIGUSR1
  ...
  //fprintf(stderr, "Perf_fd %d watchpoints tripped %lu times. i is %d\n", iter, get_trace_count(perf_fd[iter]), i);
	...
	munmap(our_mmap, 528384);
	gettimeofday(&end,NULL);
        timediff=1000000*(end.tv_sec-start.tv_sec)+(end.tv_usec-start.tv_usec);
        fprintf(stderr, "Running time %lld usecs.\n",timediff);
        fprintf(stderr, "Total time of MMAP_READ: %lld usecs.\n", readtime);
